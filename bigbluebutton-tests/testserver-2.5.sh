...
if [ -r openssl-1.0.0.cnf ]; then
   ...
   # Easy-RSA version 3 calling syntax

   sed -i 's/keyUsage = .*/keyUsage = critical, cRLSign, digitalSignature, keyCertSign/' openssl-easyrsa.cnf
   ...
   # create the server certificate and put everything where the bbb install scripts wants it
   # (might want to actually create a full chain, which this currently isn't)
   ./easyrsa --batch build-server-full test.freesoft.org nopass

   sudo mkdir -p /local/certs
   sudo cp pki/issued/test.freesoft.org.crt /local/certs/fullchain.pem
   sudo cp pki/private/test.freesoft.org.key /local/certs/privkey.pem
   sudo cp pki/ca.crt /local/certs/ca.crt
...
wget -qO- https://ubuntu.bigbluebutton.org/bbb-install-2.5.sh | sudo bash -s -- -v focal-250 -s test.freesoft.org -d -a
